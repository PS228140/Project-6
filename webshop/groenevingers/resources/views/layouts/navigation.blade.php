        ...
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 lg:flex">
                    <x-nav-link :href="route('homepage.index')">
                        {{ __('Homepage')}}
                    </x-nav-link>
                    ...
                    @if (Auth::user()->role->name === "Admin" || Auth::user()->role->name === "Accountant")
                        ...
                        <x-nav-link :href="route('orders.index')" :active="request()->routeIs('orders.index') || request()->routeIs('orders.sortOrders')">
                            {{__('Orders')}}
                        </x-nav-link>
                    @endif
                </div>
                ...
                <x-dropdown align="right" width="48">
                    <x-slot name="trigger">
                        <button class="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md text-gray-500 dark:text-gray-400 bg-white dark:bg-gray-800 hover:text-gray-700 dark:hover:text-gray-300 focus:outline-none transition ease-in-out duration-150">
                            ...
                            <img width="30px" class="ml-3" src="/storage/avatar-{{Auth::user()->id}}.png">
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden lg:hidden">
        <div class="pt-2 pb-3 space-y-1">
            ...
            @if (Auth::user()->role->name === "Admin" || Auth::user()->role->name === "Accountant")
                ...
                <x-responsive-nav-link :href="route('orders.index')" :active="request()->routeIs('orders.index') || request()->routeIs('orders.sortOrders')">
                    {{__('Orders')}}
                </x-responsive-nav-link>
            @endif
        </div>
